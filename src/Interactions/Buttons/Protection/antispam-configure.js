    ...
    async run (interaction) {
        ...
        limitCollector.on('collect', async (message) => {
            ...
            durationCollector.on('collect', async (message) => {
                ...
                if (isNaN(durationAnswer) || durationAnswer < 1 || durationAnswer % 1 !== 0) {
                    message.channel.send({
                        embeds: [
                            new MessageEmbed()
                            .setStyle('ERROR')
                            .setDescription('La durée rendu muet n\'est pas valide. Doit être exprimé en chiffres et doit se trouver au dessus de 0.')
                        ]
                    })
                    .then(async (msg) => {
                        await this.client.utils.wait(5000);
